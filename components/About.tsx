...
const EXPERTISE = [
  ...
  { icon: "⚡", label: "Micro Frontends" },
  { icon: "🎨", label: "UI/UX Engineering" },
  { icon: "📊", label: "State Management" },
  { icon: "🔧", label: "API Integration" },
];

const METRICS = [
  { num: "4.1+", label: "Years of Experience", color: "text-[var(--cyan)]" },
  { num: "12+", label: "Projects Delivered", color: "text-[var(--indigo-lt)]" },
  { num: "2", label: "Companies", color: "text-[var(--cyan)]" },
  { num: "100%", label: "Client Satisfaction", color: "text-[var(--indigo-lt)]" },
];
...
export default function About() {
  ...
  useEffect(() => {
    const items = ref.current?.querySelectorAll<HTMLElement>(".reveal, .reveal-left, .reveal-right");
    if (!items) return;
  ...
  return (
    <section id="about" ref={ref} className="py-28 bg-[var(--bg2)]">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-20 items-center">

          {/* Left */}
          <div className="reveal-left">
            <p className="font-mono text-[11px] text-[var(--cyan)] tracking-[0.12em] uppercase mb-4 flex items-center gap-2 before:content-['//'] before:opacity-50">
              About Me
            </p>
            <h2 className="font-syne font-extrabold mb-6 leading-tight"
              style={{ fontSize: "clamp(1.8rem, 3vw, 2.5rem)" }}>
              Building interfaces that{" "}
              <span className="grad-text">people love</span>
            </h2>

            <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6 mb-6 space-y-3">
              <p className="text-[var(--muted)] text-sm leading-[1.85]">
                I&apos;m <strong className="text-[var(--text)]">Madhusudhan Reddy Yeddula</strong>, a
                Frontend Developer based in Kurnool, Andhra Pradesh, with{" "}
                <span className="text-[var(--cyan)]">4+ years</span> of experience turning design specs
                into production-grade web applications.
              </p>
              <p className="text-[var(--muted)] text-sm leading-[1.85]">
                I specialize in{" "}
                <strong className="text-[var(--text)]">React.js and Next.js</strong> with deep expertise
                in TypeScript, Redux, and REST/AI API integration. Currently building AI-powered chatbots
                and real-time dashboards at Zyptr.
              </p>
              <p className="text-[var(--muted)] text-sm leading-[1.85]">
                On the backend side I&apos;m comfortable with{" "}
                <strong className="text-[var(--text)]">Node.js, Express, and MongoDB</strong>, allowing
                me to contribute across the full stack and communicate fluently with backend teams.
              </p>
            </div>
              ...
              {EXPERTISE.map((tag) => (
                <span
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs
                    bg-[var(--indigo)]/8 border border-[var(--indigo)]/20 text-[var(--indigo-lt)]
                    hover:bg-[var(--indigo)]/16 hover:border-[var(--indigo)]/40 transition-all duration-200 cursor-default"
                >
                  <span>{tag.icon}</span>
                  {tag.label}
                </span>
            ...
            {METRICS.map((m) => (
              <div
                ...
                className="bg-[var(--card)] border border-[var(--border)] rounded-2xl p-6 text-center
                  hover:border-[var(--indigo)]/50 hover:-translate-y-1 transition-all duration-250"
              >
                <div className={`font-syne font-extrabold text-4xl ${m.color}`}>{m.num}</div>
                <div className="text-xs text-[var(--muted)] mt-2">{m.label}</div>
              </div>
            ))}
          </div>
